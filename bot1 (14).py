...
async def vuln_scan_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    query = update.callback_query
    await query.answer()
    await vuln_scan(update, context)
...
async def check_link_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("Send a URL (e.g., http://192.168.8.20:80/login):")
...
async def check_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized_user(update, context):
        await update.message.reply_text("Contact admin @Imvasupareek to access the bot.")
        return
    if context.user_data.get("awaiting_checklink", False) or (context.args and len(context.args) > 0):
        url = update.message.text.strip() if context.user_data.get("awaiting_checklink") else context.args[0].strip()
        ...
        try:
            ...
            keyboard = [
                [InlineKeyboardButton(f"Brute-Force {panel_name}", callback_data=f"hunt_{ip}_{port}_{url}")],
                [InlineKeyboardButton("Main Menu", callback_data="main_menu")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
...
async def vuln_scan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not admin_pages:
        await update.message.reply_text(
            "No admin pages to scan. Run /hack or /checklink first.",
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_authorized_user(update, context):
        await update.message.reply_text("Contact admin @Imvasupareek to access the bot.")
        return
    await update.message.reply_text("Bot online! Use /start, /hack, /checklink, or /vuln.", reply_markup=main_menu_markup())
