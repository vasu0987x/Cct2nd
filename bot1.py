...
async def add_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_user.id != MAIN_ADMIN_ID:
        await update.message.reply_text("❌ Only @Imvasupareek can use /add.")
        return
...
async def remove_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_user.id != MAIN_ADMIN_ID:
        await update.message.reply_text("❌ Only @Imvasupareek can use /remove.")
        return
    if not context.args or len(context.args) != 1:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_authorized_user(update, context):
        await update.message.reply_text("Contact admin @Imvasupareek to access the bot.")
    ...
    await update.message.reply_text(
        "🎥 CCTV Hacker Bot! 🚀\n"
        "Scan CCTVs or admin panels. Choose an option below:",
...
async def start_hack_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_authorized_user(update, context):
        await update.callback_query.message.reply_text("Contact admin @Imvasupareek to access the bot.")
...
async def vuln_scan_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if admin_pages:
        ...
        return ConversationHandler.END
    await query.message.reply_text("Enter a URL (e.g., http://192.168.8.20:80/login) or IP (e.g., 192.168.8.20):")
...
async def check_link_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.message.reply_text(
        "Send a URL (e.g., http://192.168.8.20:80/login), IP (e.g., 192.168.8.20), or endpoint (e.g., /login):"
    )
...
async def help_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.message.reply_text(
        ...
        "3. Check Link: Test URLs, IPs, or endpoints\n"
        "4. Status: Bot health\n"
...
async def check_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Check if a URL, IP, or endpoint is an admin panel."""
    if not is_authorized_user(update, context):
        await update.message.reply_text("Contact admin @Imvasupareek to access the bot.")
        return
    if not (context.user_data.get("awaiting_checklink", False) or (context.args and len(context.args) > 0)):
        await update.message.reply_text(
            "Provide a URL (e.g., /checklink http://192.168.8.20:80/login), IP (e.g., 192.168.8.20), or endpoint (e.g., /login).",
    ...
    if input_text.startswith(("http://", "https://")):
        # Full URL (e.g., http://192.168.8.20:80/login)
        parsed_url = urlparse(input_text)
        logger.debug(f"Parsed URL: scheme={parsed_url.scheme}, hostname={parsed_url.hostname}, port={parsed_url.port}, path={parsed_url.path}")
        if not parsed_url.hostname:
            await update.message.reply_text("Invalid URL: No hostname found!", reply_markup=main_menu_markup())
            return
        ip = parsed_url.hostname
        if not re.match(r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$", ip):
            await update.message.reply_text("Invalid IP in URL! Use IPv4 (e.g., 192.168.8.20).", reply_markup=main_menu_markup())
            return
        port = parsed_url.port or (443 if parsed_url.scheme == "https" else 80)
        path = parsed_url.path or "/"
        protocol = parsed_url.scheme
    elif re.match(r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$", input_text):
        # IP only (e.g., 192.168.8.20)
        ip = input_text
        port = 80
        path = "/"
    elif input_text.startswith("/"):
        # Endpoint only (e.g., /login)
        ...
        await update.message.reply_text(
            "Invalid input! Use a URL (http://192.168.8.20:80/login), IP (192.168.8.20), or endpoint (/login).",
            reply_markup=main_menu_markup()
        )
        return
    ...
    try:
        if not await check_port(ip, port):
            await update.message.reply_text(f"❌ Port {port} closed on {ip}.", reply_markup=main_menu_markup())
            return
        ...
        admin_id = f"checklink_{len(context.user_data.get('admin_urls', {})) + 1}"
        context.user_data.setdefault("admin_urls", {})[admin_id] = url
        keyboard = [
            [InlineKeyboardButton(f"Brute-Force {panel_name}", callback_data=f"hunt_{ip}_{port}_{admin_id}")],
            [InlineKeyboardButton("Main Menu", callback_data="main_menu")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        if is_admin:
            ...
            await update.message.reply_text(
                f"{geo_text}❌ No admin panel at {url}.\nDetails: {', '.join(details)}",
                reply_markup=reply_markup
            )
...
async def vuln_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if input_text.startswith(("http://", "https://")):
        ...
        await update.message.reply_text("Enter port for vuln scan (e.g., 80, 80-443, 80,443,8080) or leave blank:")
        return VULN_PORT
    else:
        await update.message.reply_text(
            "Invalid input! Use a URL (e.g., http://192.168.8.20:80/login) or IP (e.g., 192.168.8.20).",
            reply_markup=main_menu_markup()
        )
        return VULN_INPUT
...
async def vuln_port(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if port_input:
        try:
            ...
            await update.message.reply_text(
                f"Invalid port input: {str(e)} Use 1-65535, range (e.g., 80-443), or list (e.g., 80,443).",
                reply_markup=main_menu_markup()
            )
            return VULN_PORT
    ...
    try:
        ...
        for port in ports_to_scan:
            ...
            for protocol in protocols:
                for path in ADMIN_PATHS[:5]:  # Limit paths for Koyeb
                    url = f"{protocol}://{ip}:{port}{path}"
...
async def ip(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not re.match(r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$", ip):
        await update.message.reply_text("Invalid IP! Use IPv4 (e.g., 192.168.1.1).", reply_markup=main_menu_markup())
        return IP
    context.user_data["ip"] = ip
    await update.message.reply_text("Enter port (e.g., 80, 80-443, 80,443,8080) or leave blank:")
    return PORT
        ...
        for port in ports_to_scan:
            ...
            for idx, admin_url in enumerate(port_admin_pages, start=len(context.user_data["admin_urls"]) + 1):
                ...
                inline_buttons.append([
                    ...
                    InlineKeyboardButton(f"Brute-Force {path_name}", callback_data=callback_data)
                ])
...
async def hunt_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Brute-force credentials, stop after first match."""
    if not is_authorized_user(update, context):
        await update.callback_query.message.reply_text("Contact admin @Imvasupareek to access the bot.")
        await update.callback_query.answer()
        return
