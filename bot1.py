...
async def check_link_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.message.reply_text(
        "Send a URL (e.g., http://86.103.65.158:8443/default.html), IP (e.g., 86.103.65.158), or endpoint (e.g., /default.html):"
    )
...
async def check_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not (context.user_data.get("awaiting_checklink", False) or (context.args and len(context.args) > 0)):
        await update.message.reply_text(
            "Provide a URL (e.g., /checklink http://86.103.65.158:8443/default.html), IP (e.g., 86.103.65.158), or endpoint (e.g., /default.html).",
    ...
    if input_text.startswith(("http://", "https://")):
        # Full URL (e.g., http://86.103.65.158:8443/default.html)
        ...
        if not is_valid_ipv4(ip):
            await update.message.reply_text("Invalid IP in URL! Use IPv4 (e.g., 86.103.65.158).", reply_markup=main_menu_markup())
            return
        port = parsed_url.port or (443 if parsed_url.scheme == "https" else 80)
        path = parsed_url.path or "/"
        protocol = parsed_url.scheme
    elif is_valid_ipv4(input_text):
        # IP only (e.g., 86.103.65.158)
        ip = input_text
        port = 80
        path = "/"
    elif input_text.startswith("/"):
        # Endpoint only (e.g., /default.html)
        ...
        await update.message.reply_text(
            "Invalid input! Use a URL (http://86.103.65.158:8443/default.html), IP (86.103.65.158), or endpoint (/default.html).",
            reply_markup=main_menu_markup()
        )
        return
    ...
    try:
        ...
        admin_id = f"a{len(context.user_data.get('admin_urls', {})) + 1}"  # Short admin_id
...
async def vuln_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if input_text.startswith(("http://", "https://")):
        ...
        await update.message.reply_text(
            "Invalid input! Use a URL (e.g., http://86.103.65.158:8443/default.html) or IP (e.g., 86.103.65.158).",
            reply_markup=main_menu_markup()
        )
        return VULN_INPUT
...
async def ip(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_valid_ipv4(ip):
        await update.message.reply_text("Invalid IP! Use IPv4 (e.g., 86.103.65.158).", reply_markup=main_menu_markup())
        return IP
    ...
    try:
        ...
        for port in ports_to_scan:
            ...
            for idx, admin_url in enumerate(port_admin_pages, start=len(context.user_data["admin_urls"]) + 1):
                admin_id = f"a{idx}"  # Short admin_id
