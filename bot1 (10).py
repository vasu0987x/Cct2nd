...
async def help_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.message.reply_text(
        ...
        "3. I'll check for open ports and detect login/admin pages across multiple ports.\n"
...
async def port(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if port_input:
        try:
            ...
            if not (1 <= port <= 65535):
                raise ValueError
            ports_to_scan = [port]  # Only scan provided port
        except ValueError:
    ...
    try:
        group_message = f"H.visual studio codeack Results for {ip}\n\n{results_text}"
        if admin_pages:
                    ...
                    for path in ADMIN_PATHS:
                        ...
                        async with session.get(url, timeout=5, ssl=False, allow_redirects=True) as response:
                            ...
                            if status in [200, 401, 403]:
                                ...
                                results.append(f"❌ Path {url} (status: {status})")
            except Exception as e:
