...
def detect_service(ip, port):
    if port in SERVICE_MAP:
        return SERVICE_MAP[port], "N/A", "N/A"
    try:
        url = f"http://{ip}:{port}"
        response = requests.get(url, timeout=5, allow_redirects=False)
        server = response.headers.get("Server", "unknown")
        return "http", server, "N/A"
...
async def scan_ip_port(ip, port, chat_id, update, context):
    ...
    async with scan_semaphore:
        try:
            ...
            # Check port and detect service
            port_open = check_port(ip, port)
            if not port_open:
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_ids[chat_id],
                    text=f"⚠️ Port {port} is closed on {ip}",
                    parse_mode="Markdown"
                )
                return

            service, product, version = detect_service(ip, port)
            ...
            if scan_results[chat_id]["details"]:
                services = scan_results[chat_id]["details"]
                result_text += (
                    f"Service: {services['service']}\n"
                    f"Product: {services['product']}\n"
                    f"Version: {services['version']}\n"
                )
            result_text += f"MAC Address: {scan_results[chat_id]['mac']}\n"
            if hack_details:
