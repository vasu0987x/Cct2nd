...
async def vuln_scan_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def check_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if input_text.startswith(("http://", "https://")):
        ...
        try:
            ...
            await update.message.reply_text("Invalid URL format!", reply_markup=main_menu_markup())
            return
    ...
    if not ip or not re.match(r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$", ip):
        await update.message.reply_text("Invalid IP!", reply_markup=main_menu_markup())
        return

    # Store for endpoint reuse
...
async def vuln_scan(update: Update, context: ContextTypes.DEFAULT_TYPE, message=None) -> None:
    ...
    if not admin_pages:
        reply_text = "No admin pages to scan. Run /hack or /checklink first."
        if message:
    ...
    results = []
    semaphore = asyncio.Semaphore(3)  # Reduced for Koyeb
...
async def port(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if port_input:
        try:
            # Handle range (e.g., "80-443") or list (e.g., "80,443,8080")
            if "-" in port_input:
    ...
    try:
        ...
        admin_pages = []
        inline_buttons = []
        context.user_data["admin_urls"] = {}  # Reset admin URLs for this scan
...
async def hunt_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if live_links:
        ...
        await query.message.reply_text("❌ No new links found.", reply_markup=main_menu_markup())
